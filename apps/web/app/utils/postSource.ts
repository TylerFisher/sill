...
// PostRep). `noun` is the word for the thing being linked — it isn't always a
// "post" (a Semble share is a card). `noun: null` means there's nothing to view
// (a Rocksky scrobble, a community bookmark), so no footer link renders; the
// post body already says everything. `site.standard.document` shares carry their
// content NSID here (pub.leaflet.content / app.offprint.content /
// blog.pckt.content), so each publisher resolves individually.
//
// Mirrors `postViewLabel` in `@sill/emails` (the RSS/email components are
// duplicated between that package and the web app).
