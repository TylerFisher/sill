...
import { z } from "zod";
import { deleteSession, getPasswordHash, getUserIdFromSession, getUserProfile, login, resetUserPassword, signup, verifyUserPassword, checkUserExists, deleteVerification, isCodeValid, prepareVerification, createOAuthClient, } from "@sill/auth";
import { db, password, user } from "@sill/schema";
import { sendVerificationEmail, sendWelcomeEmail, sendEmailChangeEmail, sendEmailChangeNoticeEmail, sendPasswordResetEmail, } from "@sill/emails";
...
const auth = new Hono()
    ...
    try {
        ...
        c.header("Set-Cookie", `sessionId=${session.id}; ${Object.entries(cookieOptions)
            .map(([k, v]) => `${k}=${v}`)
            .join("; ")}`);
        return c.json({
            success: true,
    ...
    .post("/signup/initiate", zValidator("json", SignupInitiateSchema), async (c) => {
