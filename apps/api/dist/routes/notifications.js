...
import { db, notificationGroup, notificationItem, } from "@sill/schema";
...
const notifications = new Hono()
    ...
    try {
        const group = await db.query.notificationGroup.findFirst({
            where: and(eq(notificationGroup.id, notificationGroupId), isNotNull(notificationGroup.feedUrl)),
            with: {
                items: {
                    limit: 40,
