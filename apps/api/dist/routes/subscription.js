...
const subscriptions = new Hono()
    ...
    try {
        const currentSubscription = await db.query.subscription.findFirst({
            where: and(eq(subscription.userId, userId), not(eq(subscription.status, "canceled"))),
            with: {
                polarProduct: true,
            },
        });
    ...
    try {
        ...
        if (payload.data.externalId) {
            ...
            console.log("[POLAR WEBHOOK] No external ID found, falling back to email matching");
            foundUsers = await db
                .update(user)
                .set({
                customerId: payload.data.id,
            ...
            console.error("[POLAR WEBHOOK] Neither external ID nor email available in payload");
            return c.json({ error: "Invalid payload" }, 400);
        }
        if (foundUsers.length === 0) {
            console.error("[POLAR WEBHOOK] Could not find user via external ID or email");
        ...
        if (payload.data.activeSubscriptions.length === 0) {
            await db
                ...
                .where(and(eq(subscription.userId, dbUser.id), eq(subscription.status, "active")));
            return c.json({ success: true });
        }
        // Update active subs
        const sillProducts = await db.query.polarProduct.findMany();
        const polarSubscription = payload.data.activeSubscriptions.find((sub) => sillProducts.some((product) => product.polarId === sub.productId));
        if (!polarSubscription) {
            console.error("[POLAR WEBHOOK] No valid subscription");
            return c.json({ error: "No valid subscription" }, 400);
        }
        const chosenProduct = sillProducts.find((product) => product.polarId === polarSubscription.productId);
        if (!chosenProduct) {
