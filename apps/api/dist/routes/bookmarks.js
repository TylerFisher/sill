...
import { db, bookmark, digestItem, } from "@sill/schema";
...
const bookmarks = new Hono()
    ...
    try {
        const bookmarkResults = await db.query.bookmark.findMany({
            where: and(eq(bookmark.userId, userId), query
                ? or(sql `${bookmark.linkUrl} ILIKE ${`%${query}%`}`, sql `${bookmark.posts}::jsonb->>'link.title' ILIKE ${`%${query}%`}`, sql `${bookmark.posts}::jsonb->>'link.description' ILIKE ${`%${query}%`}`)
                : undefined),
    ...
    try {
        ...
        if (posts.length === 0) {
            const digestEdition = await db.query.digestItem.findFirst({
                where: and(eq(digestItem.userId, userId), sql `EXISTS (
