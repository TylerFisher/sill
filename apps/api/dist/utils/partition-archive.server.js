...
import { exportPartitionToParquet, getPartitionRowCount, partitionExists, } from "./parquet-archive.server";
export async function archivePartitionToR2(partitionName, date) {
    const result = {
        success: false,
        partitionName,
        rowCount: 0,
    ...
    try {
        ...
        console.error(`Failed to archive partition ${partitionName}:`, result.error);
        return result;
    }
}
export async function getPartitionNameForDate(date) {
...
export async function getAllLinkPostPartitions() {
    try {
...
export async function archiveAllPartitionsToR2() {
    ...
    for (const partitionName of partitions) {
        ...
        results.push(result);
        if (result.success) {
            console.log(`✓ Successfully archived ${partitionName} (${result.rowCount} rows, ${result.fileSizeBytes} bytes)`);
        }
        else {
    ...
    const totalBytes = results.reduce((sum, r) => sum + (r.fileSizeBytes || 0), 0);
    console.log(`Archive summary: ${successCount}/${results.length} partitions archived successfully`);
    console.log(`Total rows archived: ${totalRows}`);
    console.log(`Total data archived: ${(totalBytes / 1024 / 1024).toFixed(2)} MB`);
    return results;
}
