...
import { db, type LinkPost } from "@sill/schema";

type LinkPostRecord = LinkPost;

// Combine LinkPost and Link data for denormalized queries
type DenormalizedRecord = LinkPostRecord & {
	linkId: string;
	linkTitle: string;
	linkDescription: string | null;
	linkImageUrl: string | null;
	linkGiftUrl: string | null;
	linkMetadata: Record<string, unknown> | null;
	linkScraped: boolean | null;
	linkPublishedDate: Date | null;
	linkAuthors: string[] | null;
	linkSiteName: string | null;
	linkTopics: string[] | null;
};
...
async function* createDatabaseStream(
	...
	while (true) {
		const batch = await db
			...
			.then((res) => res.rows as DenormalizedRecord[]);
...
type ParquetRecord = Omit<DenormalizedRecord, "linkMetadata"> & {
 ...
 */
function transformRecordForParquet(record: DenormalizedRecord): ParquetRecord {
	return {
		...record,
		// Convert JSON fields to strings for parquet storage if they exist as objects
		linkMetadata:
			typeof record.linkMetadata === "object" && record.linkMetadata
				? JSON.stringify(record.linkMetadata)
				: (record.linkMetadata as string | null),
