...
import { notificationGroup, notificationItem } from "../database/schema.server.js";
import { evaluateNotifications } from "../utils/links.server.js";
import type { NotificationQuery } from "../types.server.js";
...
const notifications = new Hono()
	...
	.delete("/groups/:groupId", zValidator("param", DeleteNotificationGroupSchema), async (c) => {
	...
	.post("/test", zValidator("json", TestNotificationsSchema), async (c) => {
		...
		try {
			const links = await evaluateNotifications(userId, queries as NotificationQuery[]);
	...
	.get("/groups/:notificationGroupId/feed", zValidator("param", GetNotificationGroupFeedSchema), async (c) => {
	...
	.post("/groups", zValidator("json", CreateNotificationGroupSchema), async (c) => {
		const userId = await getUserIdFromSession(c.req.raw);
