...
import { boolean, index, integer, json, pgEnum, pgMaterializedView, pgTable, text, time, timestamp, unique, uniqueIndex, uuid, } from "drizzle-orm/pg-core";
...
export const verification = pgTable("verification", {
    ...
    return {
        ...
        targetTypeKey: uniqueIndex("verification_target_type_key").using("btree", table.target.asc().nullsLast(), table.type.asc().nullsLast()),
    };
});
...
export const notificationGroup = pgTable("notification_group", {
    id: uuid().primaryKey().notNull(),
    name: text().notNull(),
    query: json().$type().notNull(),
    notificationType: digestType().notNull().default("email"),
    feedUrl: text(),
    seenLinks: json().$type().notNull().default([]),
    userId: uuid()
        .notNull()
        .references(() => user.id, { onDelete: "cascade" }),
    createdAt: timestamp({ precision: 3, mode: "date" })
...
export const blueskyAccount = pgTable("bluesky_account", {
    id: uuid().primaryKey().notNull(),
    ...
    return {
        didKey: uniqueIndex("bluesky_account_did_key").using("btree", table.did.asc().nullsLast()),
        handleKey: uniqueIndex("bluesky_account_handle_key").using("btree", table.handle.asc().nullsLast()),
    };
});
...
export const link = pgTable("link", {
    ...
    return {
        searchIndex: index("link_search_index").using("gin", sql `(
...
export const emailToken = pgTable("email_token", {
    ...
    return {
        userIdKey: uniqueIndex("email_token_user_id_key").using("btree", table.userId.asc().nullsLast()),
    };
});
...
export const mutePhrase = pgTable("mute_phrase", {
    id: uuid().primaryKey().notNull(),
    phrase: text().notNull(),
    ...
    return {
        ...
        userIdPhraseKey: uniqueIndex("mute_phrase_user_id_phrase_key").using("btree", table.userId.asc().nullsLast(), table.phrase.asc().nullsLast()),
    };
});
export const linkPostDenormalized = pgTable("link_post_denormalized", {
    id: uuid().primaryKey().notNull(),
    linkUrl: text()
        .notNull()
        .references(() => link.url),
    ...
    return {
        userIdIdx: index("link_post_denormalized_userId_idx").using("btree", table.userId.asc().nullsLast()),
        linkUrlIdx: index("link_post_denormalized_linkUrl_idx").using("btree", table.linkUrl.asc().nullsLast()),
        listIdIdx: index("link_post_denormalized_listId_idx").using("btree", table.listId.asc().nullsLast()),
        postDateIdx: index("link_post_denormalized_postDate_idx").using("btree", table.postDate.asc().nullsLast()),
    };
});
...
export const accountUpdateRelations = relations(accountUpdateQueue, ({ one }) => ({
    user: one(user, {
...
export const linkPostDenormalizedRelations = relations(linkPostDenormalized, ({ one }) => ({
    user: one(user, {
...
export const mastodonAccountRelations = relations(mastodonAccount, ({ one, many }) => ({
    user: one(user, {
...
export const blueskyAccountRelations = relations(blueskyAccount, ({ one, many }) => ({
    user: one(user, {
...
export const digestRssFeedRelations = relations(digestRssFeed, ({ one, many }) => ({
    digestSettings: one(digestSettings, {
...
export const notificationGroupRelations = relations(notificationGroup, ({ one, many }) => ({
    user: one(user, {
...
export const notificationItemRelations = relations(notificationItem, ({ one }) => ({
...
export const getUniqueActorsCountSql = (postMuteCondition) => sql `
  CAST(LEAST(
...
export const networkTopTenView = pgMaterializedView("network_top_ten").as((qb) => qb
    .select({
    link: {
        ...getTableColumns(link),
    },
    mostRecentPostDate: sql `max(${linkPostDenormalized.postDate})`.as("mostRecentPostDate"),
    uniqueActorsCount: getUniqueActorsCountSql(sql `1`).as("uniqueActorsCount"),
})
    .from(linkPostDenormalized)
    .innerJoin(link, eq(linkPostDenormalized.linkUrl, link.url))
    .where(gte(linkPostDenormalized.postDate, sql `now() - interval '3 hours'`))
