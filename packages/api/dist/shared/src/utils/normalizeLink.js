...
const shorteners = await readFile(join(__dirname, "shorteners.txt"), "utf-8").then((content) => content.split("\n"));
const trackingParameters = await readFile(join(__dirname, "trackingParams.txt"), "utf-8").then((content) => content.split("\n"));
export const normalizeLink = async (url) => {
    let parsed = null;
    try {
        parsed = new URL(url);
    }
    ...
    stringified = stringified.replace(parsed.origin, parsed.origin.toLocaleLowerCase());
    return stringified;
};
export const isShortenedLink = async (url) => {
    try {
...
export const isGiftLink = async (url) => {
    try {
        const parsed = new URL(url);
