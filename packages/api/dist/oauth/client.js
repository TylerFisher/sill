 ...
 */
export const createOAuthClient = async (request) => {
    if (oauthClient) {
        return oauthClient;
    }
    ...
    if (request) {
        ...
        baseUrl = isProduction ? process.env.PUBLIC_URL : "http://127.0.0.1:3000";
    }
    const privateKeyPKCS8 = Buffer.from(process.env.PRIVATE_KEY_ES256_B64, "base64").toString();
