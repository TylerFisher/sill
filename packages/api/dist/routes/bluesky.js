...
import { OAuthCallbackError, OAuthResolverError, OAuthResponseError, } from "@atproto/oauth-client-node";
    ...
    try {
        ...
        try {
            ...
            if (error instanceof OAuthCallbackError &&
                ["login_required", "consent_required"].includes(error.params.get("error") || "")) {
                if (error.state) {
            ...
            console.error("Bluesky OAuth Error (handled):", { error: String(error) });
            return c.json({
                success: true,
                account: {
                    did: oauthSession.did,
