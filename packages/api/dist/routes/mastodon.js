 ...
 */
function getAuthorizationUrl(instance, clientId) {
    const url = new URL(`https://${instance}/oauth/authorize`);
    url.searchParams.set("client_id", clientId);
    url.searchParams.set("redirect_uri", process.env.MASTODON_REDIRECT_URI);
 ...
 */
async function getAccessToken(instance, code, clientId, clientSecret) {
...
const mastodon = new Hono()
    ...
    try {
        ...
        const tokenData = await getAccessToken(dbInstance.instance, code, dbInstance.clientId, dbInstance.clientSecret);
