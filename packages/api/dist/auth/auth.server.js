...
import { password, session, subscription, termsAgreement, termsUpdate, user, } from "../database/schema.server.js";
...
export async function getUserIdFromSession(request) {
    ...
    const existingSession = await db.query.session.findFirst({
        ...
        where: and(eq(session.id, sessionId), gt(session.expirationDate, new Date())),
    });
 ...
 * @returns New session object
 */
export async function signup({ email, sentPassword, name, }) {
 ...
 */
export async function verifyUserPassword(userInfo, password) {
    let where = null;
    if (userInfo.email) {
...
export const hasAgreed = async (userId) => {
    ...
    const agreed = await db.query.termsAgreement.findFirst({
        where: and(eq(termsAgreement.termsUpdateId, latestTerms.id), eq(termsAgreement.userId, userId)),
    });
