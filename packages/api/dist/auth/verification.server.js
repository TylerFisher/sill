 ...
 */
export async function prepareVerification({ period, type, target, request, }) {
 ...
 */
export async function isCodeValid({ code, type, target, }) {
    const existingVerification = await db.query.verification.findFirst({
        where: and(eq(verification.target, target), eq(verification.type, type), or(gt(verification.expiresAt, new Date()), isNull(verification.expiresAt))),
 ...
 */
export async function deleteVerification(type, target) {
    await db
        .delete(verification)
