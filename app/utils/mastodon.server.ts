...
import type { Quote } from "masto/mastodon/entities/v1/quote.js";
import type { ShallowQuote } from "masto/mastodon/entities/v1/shallow-quote.js";
...
const processMastodonLink = async (
  ...
  const denormalized = {
    ...
    quotedPostDate: quote
      ? new Date(quote.quotedStatus?.createdAt || "")
      : undefined,
...
export const getLinksFromMastodon = async (
  ...
  try {
    ...
    const linksOnly = timeline.filter(
      (t) => t.card || t.reblog?.card || (isQuote(t.quote) && t.quote.quotedStatus?.card)
    );
